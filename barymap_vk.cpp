...
void BaryLevelsMapVK::init(ResourcesVK& res, const baryutils::BaryLevelsMap& baryMap)
{
  assert(baryMap.getLayout() != bary::ValueLayout::eUndefined);

  binding = res.createBuffer(sizeof(BaryMapData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
  ...
  data = res.createBuffer(sz, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
  ...
  for(uint32_t lvl = 0; lvl < MAX_BARYMAP_LEVELS * MAX_BARYMAP_TOPOS; lvl++)
  {
    ...
    map->levelsUni[lvl].coords                    = data.addr + levels[lvl].coordsOffset;
    map->levelsUni[lvl].meshletData               = data.addr + levels[lvl].dataOffset;
    map->levelsUni[lvl].meshletHeaders            = data.addr + levels[lvl].headersOffset;
    {
  ...
  map->levels = binding.addr + offsetof(BaryMapData, levelsUni);
  res.tempSyncSubmit(cmd);
}

void BaryLevelsMapVK::deinit(ResourcesVK& res)
{
  res.destroy(binding);
  res.destroy(data);
}
